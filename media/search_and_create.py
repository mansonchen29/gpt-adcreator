...
## hardcode search keyword for now. this pulls from output report somehow. need to figure it out. submit on button press (also add button in .html file)
test_data = {
    "search": "nature",
    "title": "Mansons test title",
    "soundtrack": "Freeflow"
}
...
def submit(data):
    ...
    with shotstack.ApiClient(configuration) as api_client:
        ...
        for index, video in enumerate(search_videos.get('videos')):
            ...
            for entry in videos:
                if entry.get('height') == 1080 or entry.get('width') == 1920:
                    hd_file = entry
